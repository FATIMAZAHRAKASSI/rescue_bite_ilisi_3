...
const SignalItem = ({ item, onPress }) => {
    ...
    const handleStartNavigation = async (item) => {
      const storedUserId = await AsyncStorage.getItem('idorganisation');
      const integer1 = parseInt(storedUserId);
      const response = await axios.get(`http://192.168.43.177:3002/get/SQLorganisation/${integer1}`, {
            headers: {
              'Content-Type': 'application/json',
            },
            data:{}
          });
console.log("response"+JSON.stringify({ latitude: response.data.latitude, longitude: response.data.longitude }))
console.log("item"+JSON.stringify({ latitude: item.latitude, longitude: item.longitude }))
      const startLocation = { latitude: response.data.latitude, longitude: response.data.longitude };
      const endLocation = { latitude: item.latitude, longitude: item.longitude };
...
const OrganisationsOrganisation = () => {
   const [mydata,setdata]=useState([]);
    ...
    const handlePressSignal = (item) => {
     //   navigation.navigate('DetailsSignaux', { item: item }); // Envoyer les données de l'élément sélectionné à DetailsSignaux
      ...
      const integer1 = parseInt(storedUserId);
      try {
         const response = await axios.get(`http://192.168.43.177:3005/findDemandesAcceptesOrga/${integer1}`, {
            headers: {
              'Content-Type': 'application/json',
            },
            data:{}
        ...
        <View style={styles.container}>
            <View style={{ marginVertical: 22 }}>
                ...
                <Text style={{
                    fontSize: 16,
                    color: COLORS.black
                }}>List of Accepted Signals of Organisation </Text>
            </View>
            <FlatList
                data={mydata}
...
export default OrganisationsOrganisation;
