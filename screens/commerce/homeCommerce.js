...
function WelcomeCommerce() {
  ...
  const handleMarkerPress = (marker) => {
   ...
   navigation.navigate('DetailsOrganisation', { markerId: marker.id });
 };
 ...
 const fetchData = async  () => {
   ...
   try {
      const response = await axios.get(`http://192.168.43.177:3002/FindAll/SQLorganisation`, {
         headers: {
       ...
       if(response.status==200)
       {
         ...
         response.data.forEach(async item => {
            ...
            const merge={...jsonorga,"id":item.id_utilisateur,"title":item.description}
