...
app.post('/create/SQLorganisation', async (req, res) => {
  try {
    const data = req.body;
   // const data = req.body.body;
   const responseComerceDB = await axios.post('http://localhost:3000/create/SQLorganisation/organisation', data, {
      headers: {
...
async function authentifier(email: any,pass:any) {
  try {
      ...
      const response = await axios.get('http://localhost:3000/findByFilter/SQLorganisation/organisation', {
          headers: {
            'Content-Type': 'application/json',
          },
          data:{
...
app.get('/authentificationOrganisation/:email/:password', async (req :any, res:any) => {
...
app.get('/get/SQLorganisation/:id', async (req :any, res:any) => {
  const id=req.params.id;
  console.log("votre id"+id);

  const response = await axios.get(`http://localhost:3000/get/SQLorganisation/organisation/${id}`, {
          headers: {
            'Content-Type': 'application/json',
          },
          data:{}
...
console.log("hihi"+JSON.stringify(mergedJson))
      res.send(mergedJson);
      }
  else
  {
...
app.get('/FindAll/SQLorganisation', async (req :any, res:any) => {
  const response = await axios.get(`http://localhost:3000/FindAll/SQLorganisation/organisation`, {
          headers: {
            'Content-Type': 'application/json',
          },
          data:{}
        ...
        {  
          ...
          if (Array.isArray(response.data)) {
            ...
            for(var item of response.data){
             ...
             const response2 = await axios.get(`http://localhost:3002/get/SQLorganisation/${item.id_utilisateur}`, {
            headers: {
              'Content-Type': 'application/json',
            },
            data:{}
