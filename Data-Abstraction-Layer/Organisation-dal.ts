...
export default class OrganisationDal implements DatabaseAbstractionLayer {
    ...
    async connect() {
        let connection;
        try {
             connection = await this.mysqlConnection.getConnection();
            ...
            // Assuming `entityName` is 'utilisateur' for the user table and 'organisation' for the organization table
               ...
               if (userResult!=null) {
                ...
                const orgaresult:any=await this.mysqlConnection.executeQuery(
                        'INSERT INTO organisation (type_organisation, id_utilisateur) VALUES (?, ?)',
                        [data.type_organisation, userResult.insertId]
                    );
               return orgaresult;
            } else {
    ...
    async get(entityName: string, id: any): Promise<any> {
        try {
            ...
            await this.connect();
        const userResult : any = await this.mysqlConnection.executeQuery(
            `select * from utilisateur,organisation 
            where utilisateur.id_utilisateur=organisation.id_utilisateur
            AND organisation.id_utilisateur=${id}`,
            [id]
        );
        const imageResult : any = await this.mysqlConnection.executeQuery(
    ...
    async findAll(entityName: string): Promise<any[]> {
        try {
            ...
            await this.connect();
        const userResult : any = await this.mysqlConnection.executeQuery(
            `select * from organisation,utilisateur
        ...
        try {
           ...
           { console.log("your json"+JSON.stringify(userResult[0].id_utilisateur))
            const organisationresult : any = await this.mysqlConnection.executeQuery(
                    'SELECT * FROM  organisation where id_utilisateur = ? ',
